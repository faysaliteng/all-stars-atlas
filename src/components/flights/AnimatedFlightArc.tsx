...
export default function AnimatedFlightArc({ height = 12, direction = "departure", compact = false }: AnimatedFlightArcProps) {
  ...
  // Colors
  const departColor = "#2563eb"; // royal blue
  const returnColor = "#7c3aed"; // purple for return
  ...
  return (
    <div className="w-full relative flex items-center" style={{ height: `${h}px` }}>
      ...
      {/* Arrow track */}
      <div className="flex-1 relative mx-0.5 overflow-hidden flex items-center" style={{ height: h }}>
        {/* Animated arrows */}
        <div
          className="absolute inset-0 flex items-center"
          style={{
            animation: isReturn
              ? 'arrow-scroll-left 1.8s linear infinite'
              : 'arrow-scroll-right 1.8s linear infinite',
          }}
        >
        ...
        {/* Fade edges */}
        <div className="absolute inset-y-0 left-0 w-5 bg-gradient-to-r from-card to-transparent z-10 pointer-events-none" />
        <div className="absolute inset-y-0 right-0 w-5 bg-gradient-to-l from-card to-transparent z-10 pointer-events-none" />
